...
export default function OrderHistoryPage() {
  ...
  const openModal = async (order: OrderItem) => {
    try {
      ...
      setEditTableId(res.data.cafeTable?.id || null);
      setIsModalOpen(true);
    } catch {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Xem, sửa và quản lý toàn bộ lịch sử đơn hàng</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl animate-in">
            ...
            <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-slate-900 rounded-xl font-bold flex items-center gap-2 transition-colors shadow-lg shadow-amber-500/20"
              >
                <Save size={18} />
